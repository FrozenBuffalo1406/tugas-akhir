...
String getTimestamp() {
    ...
    if (!getLocalTime(&timeinfo)) {
        Serial.println("[TIME-ERROR] Gagal mendapatkan waktu lokal.");
...
void sendDataToServer(const char* url, const char* deviceId, const char* timestamp, float* beatBuffer, float* afibBuffer, int length) {
    if (WiFi.status() == WL_CONNECTED) {
        ...
        // [PERBAIKAN] Logika "Fire and Forget"
        if (httpCode > 0) {
            // Kode balasan positif (2xx) berarti request sudah diterima server. Cukup.
            Serial.printf("[WIFI-SERVER] Data berhasil dikirim (Kode: %d)\n", httpCode);
            // Kita tidak lagi memanggil http.getString() yang akan menunggu balasan
        } else {
            // Kode balasan negatif berarti request bahkan gagal sampai ke server.
